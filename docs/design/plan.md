# Distributed Message Bus — Execution Plan v1.0
...
following the publish-subscribe pattern. The design is **Kafka-inspired**: a partitioned append-only log
with offset-based consumption and consumer groups, so offset, partition, and consumer-group semantics
follow Kafka conventions (see Section 2 and DEC-023 in the decision log).
...
| Consumer group | **M3** — in-group partition assignment + distribution (static). Split: **M3a** static assignment + manual batch commit + resume (*done, merged*); **M3b** redelivery + DLQ (*done, merged*); **M3c** rebalance / dynamic assignment (*done, merged*) | Rebalancing maturity | Splits a topic across group members; built on top of M2's partitions, NOT part of M2 |
...
| v1.3 | **M4 step 3 done — mTLS mutual authentication + identity extraction; merged. M4 complete.** Broker and client present CA-signed certs and verify each other; the broker extracts the client identity (`User:<CN>`) onto the request context (`ServerCallContext.UserState`) for a future ACL to read — **authentication + identity only, no authorization, no proto change**. Delivered in five checkpointed slices: a committed **certgen** tool (built-in X509 APIs, no OpenSSL — CA/broker/client/untrusted, per §7.1), an **opt-in mTLS listener** (`Broker:RequireClientCertificate` default off, DEC-030 seam pattern), a **`ClientCertificateValidator`** (chain **and** clientAuth-EKU validation), an **identity interceptor**, and **accept/reject integration tests**; a separate CI cert step proved unnecessary (the harness self-generates in-process). Key outcomes in the decision-and-fix log: **FIX-023** (validation checked the chain but not the EKU — a CA-signed *server* cert would have passed as a client cert; found by the orchestrator's independent code read after the CLI and the review sub-agent both missed it), **FIX-024** (reject tests asserted a flaky gRPC StatusCode → assert the stable inner exception instead; the mTLS instance of DEC-031), **DEC-033** (ACL is a separate later milestone; M4 lays the interceptor seam + the M5 no-bypass guard), **DEC-034** (principal shape `User:<CN>` in UserState under a fixed key), **DEC-035** (mTLS switch defaults off; existing tests unchanged), **DEC-036** (verification split: unit for extraction, integration for transport, e2e deferred to ACL), **DEC-037** (security config fails fast — never a silent downgrade; contrast DEC-030's numeric fallback). **Deferred (in 09's Ledger):** a `Subscribe_Latest_Atomicity` unit test flakes on the upgraded test infra (xunit runner 2→3, test-sdk 17→18) — test-only marker-technique fix pending before merging the Dependabot test-stack PR. Next: M5 (throughput optimization — must honor the DEC-033 no-bypass guard). |
