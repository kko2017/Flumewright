...
- [DEC-027 — Tool boundary: interleaving races → Coyote; sweeper + wall-clock liveness → integration tests 🔒](#dec-027--tool-boundary-interleaving-races--coyote-sweeper--wall-clock-liveness--integration-tests-)
- [DEC-028 — Shift-left the SonarCloud rules into the local build (planned, execute at M4 start) 🔒](#dec-028--shift-left-the-sonarcloud-rules-into-the-local-build-planned-execute-at-m4-start-)
- [DEC-029 — Shift-left analyzer tiering: mechanical rules hard-gate locally; semantic rules are SonarCloud + human review, never build-gated 🔒](#dec-029--shift-left-analyzer-tiering-mechanical-rules-hard-gate-locally-semantic-rules-are-sonarcloud--human-review-never-build-gated-)
...
- [FIX-019 — Coyote was coupled to every build (`coyote: not found` broke CI); decoupled into a dedicated, manifest-pinned step 🔒](#fix-019--coyote-was-coupled-to-every-build-coyote-not-found-broke-ci-decoupled-into-a-dedicated-manifest-pinned-step-)
...
## FIX-019 — Coyote was coupled to every build (`coyote: not found` broke CI); decoupled into a dedicated, manifest-pinned step 🔒

**Where:** `tests/Flumewright.ConcurrencyTests/Flumewright.ConcurrencyTests.csproj`, `.config/dotnet-tools.json`, `ci.yml`. Surfaced on the first PR CI run for M3c, at the SonarCloud analyze step.

**Symptom.** CI failed with `coyote: not found` (exit 127). The concurrency-test project carried an MSBuild target wired `AfterTargets="Build"` that invoked `coyote rewrite` on *every* build. Locally the Coyote CLI tool happened to be on `PATH`; the CI runner had no such global tool, so any build that touched the project died.

**Cause.** The Coyote *rewrite* (the binary-rewriting step that makes interleaving exploration actually work, see FIX-016) was coupled to the ordinary build. That is wrong twice over: (1) every consumer of the build — `dotnet build`, the pre-commit hook, the SonarCloud analyze — was forced to have the Coyote CLI present; (2) the `Microsoft.Coyote.Test` package alone does **not** auto-rewrite, so without the explicit `coyote rewrite` the suite silently drops to one iteration — exactly the FIX-016 false-green signature. Coupling the rewrite to the build conflated "compile the tests" with "prepare the systematic-testing instrument."

**Fix (full decouple).** Three moves: (1) a `.config/dotnet-tools.json` manifest pins `microsoft.coyote.cli` to a known version, restored explicitly via `dotnet tool restore` wherever Coyote runs — no reliance on an ambient global tool; (2) the rewrite target dropped `AfterTargets="Build"`, so an ordinary build no longer rewrites — the target is now invoked explicitly (`dotnet build -t:CoyoteRewriteTarget`); (3) `ci.yml` gained a dedicated **"Run Coyote Tests"** step (restore tool → build with the rewrite target → `dotnet test --no-build`), while the ordinary `dotnet test` excludes the Coyote assembly by filter. A documented side benefit: the pre-commit hook (build + fast unit tests) is now automatically Coyote-free.

**Lesson.** A systematic-testing instrument must be a *deliberate, explicit* step, never a silent rider on the build. The FIX-016 follow-through: it is not enough to make Coyote run — the run must be reproducible on a clean machine (pinned tool) and impossible to accidentally reduce to the no-op build path.

## FIX-020 — A CI iteration guard so a future regression can't silently drop Coyote to one iteration 🔒

**Where:** `ci.yml` (the "Run Coyote Tests" step). A direct hardening of the FIX-016 / FIX-019 lesson.

**Symptom / risk.** FIX-016 showed that a Coyote suite which is not rewritten still *passes* — it just explores one interleaving instead of ~100, verifying nothing. FIX-019 fixed the wiring, but nothing yet *enforced* that the rewrite stayed effective; a future edit to the pipeline could quietly reintroduce the one-iteration trap, and CI would stay green.

**Fix.** The Coyote step now tees `--logger "console;verbosity=detailed"` output to a file (with `set -o pipefail`) and greps the report: **fail the build** if fewer than the expected number of tests report "Explored 100 execution paths", OR if *any* test reports "Explored 1 execution path". The check is verification-of-the-verifier: it asserts the instrument did real work, not merely that the tests passed.

**Both failure directions were exercised.** The happy path (rewrite present → 5 tests × ~100 paths) passes the guard; a deliberately un-rewritten run (which produces "Explored 1 execution path" lines) was confirmed to make the guard *fail* with a non-zero exit. So the guard is itself tested in both directions — the FIX-016 lesson made mechanical and permanent.

**Lesson.** When a test layer can fail open (pass while doing nothing), add a machine check that the layer actually ran, and prove that check fails when the layer is disabled. A guard you haven't watched fail is itself unverified.

## FIX-021 — A CI-only e2e failure that looked like a rebalance defect but was a flaky test against correct product behaviour 🔒

**Where:** `tests/Flumewright.IntegrationTests/DynamicRebalanceE2ETests.cs` (`MembershipLifecycle_RedistributesOnJoinAndLeave`). The most instructive episode of the M3c wrap — recorded in full, including the wrong turns, because the *process* is the lesson.

**Symptom.** `MembershipLifecycle` (five sequential rebalance stages in one test) passed locally in ~4s but timed out on CI, consuming its full budget. A consumed timeout — not headroom — so it read as a real defect.

**The false trail (and how it was caught).** The first diagnosis was a product bug: a member whose `JoinGroupAsync` is cancelled mid-rebalance lingering as a "ghost" holding partitions (the cancel handler evicts only `!HasJoined` members, but the caller set `HasJoined = true` early). This was validated three ways — own analysis, the CLI's own pass, and an isolated `code-review` sub-agent — which *agreed*, and the isolated reviewer added a sharper finding: if the ghost was the leader, the group could deadlock (the dead leader never runs `SyncGroupAsync`, survivors wait forever on `SyncTcs`). It matched the real Kafka bugs **KAFKA-9752 / KAFKA-7610** (a disconnected member left in the group; group stuck in PreparingRebalance). Convincing — but wrong as a *product* conclusion. While attempting the "fix," the CLI modified happy-path join logic (an early-bailout, a `delayTcs`/`Task.WhenAny` rewrite), hit a Coyote-reported hang on the new `_Leader` test, and was about to silence it with a try/catch — the fake-green reflex (FIX-012/018). That work was discarded with `git reset --hard` (local-only; never pushed — see the safety note below).

**The confirmation diagnosis (what was actually true).** A focused investigation of the sweeper settled it: **the product is correct.** `SweepDeadMembers` evicts on `now - LastHeartbeat > sessionTimeout` with no state shield, so a cancelled/disconnected member — including a ghost *leader* — is always evicted after the session timeout, and the eviction's `StartRebalance` calls `SyncTcs.TrySetCanceled`, unblocking the survivors. The cluster self-heals exactly as designed (the KIP-62 sweeper). The CI failure was a **test artifact**: `MembershipLifecycle` called `LeaveGroupAsync` *before* cancelling the consumer task, so c1's still-running heartbeat could spuriously re-join in the gap — accidentally modelling an *ungraceful* disconnect, which intentionally costs one ~10s session-timeout penalty. Across five stages on a slow runner, 5 × ~10s exceeded the budget. Locally, cancellation won the race, c1 left gracefully, and the penalty never occurred (~4s).

**The leader-deadlock that wasn't reproducible in Coyote — the tool-boundary lesson.** The `_Leader` Coyote test "hang" that sent the fix off the rails was itself a category error: it modelled a member that vanishes and never returns, with **no sweeper and no clock** (Coyote's world). Coyote correctly reports an unresolvable hang there — because the external actor that resolves it (the sweeper, over wall-clock time) does not exist in Coyote. Coyote explores *interleavings*; leader-vanish recovery is a *liveness property resolved by an external actor over time*. Forcing it into Coyote produced the rabbit-hole. (Captured as a durable rule in DEC-027 and in the Concurrency Strategy doc.)

**Fix (test-only; product untouched).** First, `MembershipLifecycle` was made to leave gracefully (cancel the consumer/heartbeat loop and await it *before* `LeaveGroupAsync`), removing the spurious re-join. That fixed the original failure but the test remained structurally timing-sensitive (a later CI run failed at its first stage, c1 alone receiving all four partitions — a `Latest`-reset reception race, not the product). A short, multi-angle re-diagnosis (own pass + isolated sub-agent) ruled out a product cause; one wrong hypothesis (empty-key publish routing all to one partition) was rejected fast by reading the partitioner (empty key is explicitly round-robin). Conclusion: this composite e2e is inherently fragile on CI. **Both flaky e2e tests were removed** — the briefly-added `Liveness_LeaderVanish_IsEvicted` (also flaky on CI for the same wall-clock reason) and `MembershipLifecycle` itself — with their now-unused helpers (`ConsumeAsync`, `EnsureReceivesFrom`) and fields. The behaviours they touched remain covered by stable tests: `HandoverSafety_StartsFromCommittedOffset` (leave → handover from committed offset), `Liveness_DeadMember_IsEvicted` (sweeper eviction → another member receives), `Liveness_SlowHandler_NotEvicted`, and `ZombieFencing_StaleGenerationCommit_Rejected`. The integration suite went 19 → 17 and is now green on CI. **No `src/` change was made for FIX-021** — the diagnosis's whole value was *not* changing correct product code.

**The M4 gap (recorded so it isn't lost).** Two behaviours are no longer directly asserted: the dynamic-join partition *split* ({0,1,2,3} → {0,1}/{2,3} when a second member joins), and a sequential multi-stage join/leave flow. Plus leader-vanish self-heal deserves a proper home. These will be rebuilt in M4 as **stage-isolated** integration tests (one rebalance per test, deterministic reception), and leader-vanish as an integration test with the real sweeper — see the Deferred Items Ledger.

**Lessons.** (1) A consumed-timeout e2e is a symptom, not a diagnosis — confirm whether the system *self-heals* before declaring a product bug; here the sweeper made a scary-looking "ghost/deadlock" a non-issue. (2) Use each verification tool within its boundary: Coyote for interleavings, integration tests for sweeper + wall-clock liveness (DEC-027). (3) Multi-angle validation cuts both ways — three reviewers agreed on a *wrong* product conclusion; it was the confirmation diagnosis, not consensus, that was decisive. A wrong hypothesis (empty-key) was still useful: reading the code to reject it was cheap. (4) "Local green, CI fail" recurred yet again (cf FIX-016) — slowness widens races; a test that depends on winning a race is the bug. (5) The local-git-only rule contained the accident: the bad happy-path commit never reached the remote, so `git reset --hard` erased it cleanly.

## DEC-027 — Tool boundary: interleaving races → Coyote; sweeper + wall-clock liveness → integration tests 🔒

A durable rule distilled from the FIX-021 rabbit-hole, so the boundary isn't re-crossed.

- **Coyote (Layer 5)** explores *interleavings* of in-process operations with no background services and no real clock. It is the right tool for races that are resolved (or violated) purely by the order of operations: ghost/duplicate members, generation fencing, cross-component lock ordering, cancel-vs-join schedules. Coyote's world deliberately has no sweeper and no time.
- **Integration tests (Layer 4)** run the real broker with the real `GroupCoordinatorSweeperService` and real wall-clock time. They are the *only* correct place to verify **liveness properties resolved by an external actor over time** — e.g. a vanished member (or leader) being evicted by the session-timeout sweeper so the group recovers.
- **The failure mode to avoid:** forcing a sweeper-dependent liveness scenario into Coyote. Coyote will (correctly) report an unresolvable hang, because the actor that would resolve it does not exist in that world. That report is *not* a product bug — it is a category error in test placement. (This is exactly what derailed the FIX-021 fix attempt.)
- **Practically:** when a scenario's recovery sentence contains "…until the sweeper…" or "…after the session timeout…", it is an integration test, not a Coyote test.

## DEC-028 — Shift-left the SonarCloud rules into the local build (planned, execute at M4 start) 🔒

The M3c wrap spent several commits on SonarCloud findings discovered *after* push (Batch A/B/C + stragglers). That is post-hoc: the analyzer ran in CI, not on the developer's machine, so issues surfaced a full round-trip late.

**Decision (not executed in M3c, to avoid entangling the wrap):** at M4 start, on the clean post-merge base, add an `.editorconfig` that pulls the relevant SonarAnalyzer rules into the local build and the pre-commit hook, so the same findings appear at write/commit time rather than in CI — and add the coding conventions to `GEMINI.md` so the agent writes to them from the start. This trades a one-time setup for the elimination of the post-hoc SonarCloud cleanup tail we saw in M3c. Deliberately deferred (not applied now) because changing analyzer wiring mid-wrap would have entangled the merge; recorded here and in the Deferred Items Ledger so it is picked up at the M4 boundary.

## DEC-029 — Shift-left analyzer tiering: mechanical rules hard-gate locally; semantic rules are SonarCloud + human review, never build-gated 🔒

The execution detail of DEC-028. The goal: the ~44 SonarCloud findings each milestone were ~30–35 mechanical "make-it-static" items; rather than clean those post-hoc every time, gate them locally so the agent writes clean of them and they never reach SonarCloud as noise. Rules are split into two tiers by whether fixing them needs **semantic judgment**.

**Rule-ID prefixes (so future readers aren't decoding bare numbers):** `S####` = **SonarQube / SonarCloud** (the `SonarAnalyzer.CSharp` package, and what the CI SonarCloud scan reports); `CA####` = **.NET Code Analysis** (built into the SDK, on via `EnableNETAnalyzers`); `IDE####` = **Roslyn IDE / code-style** analyzers (SDK built-in, run on build only with `EnforceCodeStyleInBuild`); `VSTHRD###` = **Microsoft.VisualStudio.Threading.Analyzers** (a separate package — the static-analysis layer of the concurrency strategy).

**Tier 1 — local hard gate (build `error`).** Only rules whose fix can **never change behaviour** — mechanical, obvious, no judgment, no harmful workaround. **As implemented:** **CA1822** (member uses no instance state → make it `static`) and **CA1861** (hoist a constant array argument to a `static readonly` field). Both are .NET built-in (no extra package), set to `error` in `.editorconfig`. CA1822 is the .NET equivalent of Sonar's **S2325** — the static findings that were the bulk of M3c's noise — so the same cleanup is enforced locally without needing the Sonar package at all.

**Why no SonarAnalyzer package locally, and why these two only.** It took three wrong attempts to learn this (recorded so we don't repeat them): (1) adding `SonarAnalyzer.CSharp` locally, with `TreatWarningsAsErrors=true` already set, promoted *hundreds* of Sonar `warning`s to build errors; the agent "fixed" that by setting ~17 rules to `severity = none` — including **S108** (empty block) and **S2699** (assertion-less test), exactly the fake-green signals we care about — which is the evasion this very decision warns against, and would make us see *less* than M3c. (2) `CodeAnalysisTreatWarningsAsErrors=false` does NOT exempt third-party analyzers (only Microsoft's `CA####`), so Sonar rules still errored. (3) A bulk `dotnet_analyzer_diagnostic.severity = suggestion` would demote everything — including the **VSTHRD** concurrency rules, which the vs-threading docs explicitly warn against silencing. The realization that ended the loop: **Tier 1 is fully covered by the .NET built-in analyzers (CA1822, CA1861); Tier 2 was always meant for SonarCloud + human review, so it never needed to be local.** Therefore the Sonar package is not referenced locally at all — no promotion problem, no rule-disabling, nothing.

**IDE0005 (unused `using`) was considered for Tier 1 and rejected.** Three reasons: (a) **off-target** — SonarCloud does not flag unused usings (M3c passed with 25 of them), so gating it does nothing for the "reduce SonarCloud noise" goal; (b) **not safely auto-fixable** — a using that looks unused in one build config may be required under `#if`/conditional compilation, and a build-error gate would pressure the agent to delete it without checking; (c) it **violates the Tier 1 principle** — removing such a using *can* change behaviour (breaks another config), so by the same rule that keeps S1066/S3440 out of Tier 1, IDE0005 is not Tier 1. It is simply not enforced (the 25 existing ones are left alone).

**Tier 2 — surfaced only, never build-gated (SonarCloud + human review).** Rules needing semantic judgment, or where a build gate would invite a harmful workaround: **S1066** (mergeable nested `if` — NOT Tier 1: merging changes short-circuit/evaluation order, and nested ifs sometimes encode atomicity/ordering intent in concurrent code), **S3440** (useless conditional assignment — NOT Tier 1: a property setter can have side effects, so `if(x!=v)x=v` vs `x=v` may differ), **CA1031 / S112** (general `catch (Exception)` / `throw new Exception`), **S2139** (catch-log-rethrow), **S3776** (cognitive complexity), and the rest of the exception-handling / "intentional and reviewed" family. These are **not configured locally** (no Sonar package) — they are caught by the CI SonarCloud scan and reviewed by human + orchestrator. That is by design, not omission: Tier 2 was always CI's job.

**Why Tier 2 must not break the build (the crux).** FIX-010 / FIX-012 / FIX-018 taught that swallowed or over-broad exception handling causes *fake-green*. If a general-`catch` rule were a build **error**, the pressure to pass the build would push the agent toward a bad workaround — narrowing a catch so the exception escapes at runtime, blanket `#pragma` suppression, swallowing the fault another way, or (for S1066) merging ifs that were intentionally separate. Exception handling is a *meaning* judgment: a general catch can be a fault-swallowing bug OR a legitimate, documented cleanup (cf the three `// [suppress: reason]` catches kept in FIX-018). So the machine *flags* it (SonarCloud) and a human + orchestrator decide. Same logic for S3776: in M3c we deliberately kept `JoinGroupAsync` / `RunSingleGenerationAsync` complex (concurrency-critical, Coyote-verified, suppress-with-reason) — a build error there could push a dangerous refactor of a verified critical section.

**VSTHRD is the concurrency static-analysis layer — handled with care, not bulk-demoted.** The `Microsoft.VisualStudio.Threading.Analyzers` package stays exactly as-is, and its semantic rules (**VSTHRD003** awaiting foreign tasks, **VSTHRD103** sync-blocking in async, **VSTHRD110** unobserved tasks, etc.) remain active build-wide under `TreatWarningsAsErrors`. The one exception is **VSTHRD200** (async methods should end in `Async`), which is a pure *naming-style* rule, not a concurrency-safety rule. It is suppressed **for test projects only** (`[tests/**/*.cs]`, with an intent comment), because test methods use descriptive names (`MembershipLifecycle_RedistributesOnJoinAndLeave`), not an `Async` suffix; the 44 test-method names are NOT renamed. This is the distinction behind the "no `severity = none`" rule: silencing a *meaningful* rule to pass the build is evasion (the 17-rule incident); a documented, scoped suppression of a *naming-style* rule for a legitimate reason is a normal decision. (The orchestrator briefly mis-told the agent to remove this suppression as if it were a leftover — it is intentional and was restored, improved from global to test-scoped.)

**Scope (DEC: 가).** Start focused — the rules actually encountered across M3c, via the .NET SDK's built-in analyzers — not the whole "Sonar way" profile. Expand later if warranted (e.g. promoting a specific VSTHRD rule to a hard gate during concurrency-heavy work). `GEMINI.md` carries the tiering principle (Tier 1 only if a fix can never change behaviour) and the exception-handling rule of thumb (never a bare fault-swallowing catch; intentional broad/empty catch carries `// [suppress: <reason>]`; general catches are reviewed, not auto-fixed) so the agent writes mechanical-clean from the start and Tier 1 rarely fires.

## FIX-022 — Rebuilt the flaky composite e2e tests (FIX-021 gap closed): stage-isolated, deterministic, real-sweeper liveness 🔒

**Symptom.** FIX-021 removed two composite rebalance e2e tests (`MembershipLifecycle_RedistributesOnJoinAndLeave`, `Liveness_LeaderVanish_IsEvicted`) as structurally timing-fragile on CI, dropping the integration suite 19 → 17 and leaving a recorded M4 gap. The behaviours (dynamic-join partition split, leader-vanish self-heal) were uncovered until rebuilt.

**Cause.** The originals baked in three fragility patterns: (1) ~5 sequential rebalance stages in one test, so per-stage slowness compounded; (2) a `Latest`-offset reception race — the test depended on winning a timing race for which member received which records (a slow CI runner let c1 receive all four partitions); (3) long real-time waits against a ~10s wall-clock session-timeout penalty × several stages, exceeding CI budget.

**Fix.** Rebuilt as four stage-isolated integration tests, one rebalance each, on `test/m4-rebuild-e2e` (own branch, per-file commits). **(a) dynamic-join partition split** — leader computes the split via `RangeAssignmentStrategy`; ownership is asserted by reading `SyncGroupResponse.Assignments` back and comparing to independent constants ({0,1}/{2,3}), NOT by dictating partitions — so the split algorithm is genuinely exercised (early drafts subscribed to hardcoded partitions and asserted they arrived, which was tautological fake-green; caught at checkpoint and reworked). **(b) leader-vanish self-heal** — the first-join leader is silenced (ungraceful, no `LeaveGroup`); the real sweeper evicts it on the shortened timeout (DEC-030); the test asserts the vanished member *was* the leader and the new leader is the former follower, so it pressures re-election (distinct from `Liveness_DeadMember`). **(c) `Latest` semantics, single member** — pre-subscription messages skipped, only post-subscription read. **(d) `Latest` × rebalance, per member** — each new owner's cursor resets at log-end; pre-subscription messages on newly-assigned partitions are skipped (see DEC-031 for the per-member stream-activation technique). Every test wraps each `MoveNext` in the per-test `cts` deadline (a hang becomes a clean timeout, FIX-008 discipline). Two pre-existing liveness tests (`Liveness_SlowHandler_NotEvicted`, `Liveness_DeadMember_IsEvicted`) had their 12s waits reduced to 2s to match the new 1s test timeout — meaning preserved because heartbeats run on a background task independent of the handler, so "handler-stall > session-timeout, member survives" holds identically at 2s>1s as at 12s>10s.

**Verification.** Each new/changed test ran 10× in a loop, flake-free (the whole point). Integration suite 17 → 21. Unit suite green. Coyote suite provably untouched (verified by `git diff` of the branch: only `GroupCoordinatorSweeperService.cs`, `BrokerAppFactory.cs`, `DynamicRebalanceE2ETests.cs` changed; `Broker.Core/`, `ConcurrencyTests/`, `coyote.json` are empty diffs — `GroupCoordinator.cs` byte-identical). SonarCloud gate initially failed (see DEC-030 fallback-coverage note) and was resolved by testing the fallback, not by lowering the threshold.

**Future impact.** Closes the FIX-021 Deferred Ledger row. The `.Partition`-set-as-ownership observation and the "observe, don't dictate" assertion shape are reusable for any future rebalance test. Note: the {0,1}/{2,3} assignment in test (d) is *setup, not the assertion* — split correctness is owned by test (a); do not mistake (d)'s hardcoded assignment for the (a) pattern. mTLS (M4 step 3) sits on top of a now-restored, flake-free rebalance suite.

---

## DEC-030 — Sweeper timing (session-timeout / sweep-interval) is a configurable seam for testability; wall-clock dependency kept, the clock shortened 🔒

**Context / decision.** `GroupCoordinatorSweeperService` hardcoded `_sessionTimeout = 10s` and `_sweepInterval = 2s` as `readonly` fields, forcing liveness tests into 12s wall-clock waits — the exact CI-budget fragility FIX-021 flagged. Made both durations configurable (`Broker:SessionTimeoutSeconds` / `Broker:SweepIntervalSeconds` via `IConfiguration`), **defaulting to 10s/2s when the keys are absent**, so production behaviour is unchanged. Tests inject short values (1s / 250ms) via `BrokerAppFactory` for the whole integration suite.

**Why this is a testability seam, not a bug fix (and not the DEC-027 boundary being violated).** The product is correct; the sweeper's evict-then-`StartRebalance` logic is untouched — only the *source* of the two durations changed. DEC-027 says sweeper + wall-clock liveness belongs to the integration layer (not Coyote); this decision keeps that placement but makes the wall clock *short* so the real-sweeper test runs in ~1–2s instead of ~12s. We do NOT remove the wall-clock dependency (that would defeat the point — leader-vanish self-heal IS a wall-clock property); we shrink the penalty's magnitude. This is orthogonal to removing the reception race (`Earliest` / ordering does that); here we only cut the wait.

**Fallback + coverage (the SonarCloud sub-lesson).** The Finding-1 review added a guard: a non-positive config value falls back to the default (rather than silently clamping via `Math.Max`, which hides typos, or throwing, which would kill startup on a typo). SonarCloud then failed the New-Code coverage gate at 77.8% with **Uncovered Conditions: 2, Uncovered Lines: 0** — the two fallback branches, never hit because tests only inject valid positives. Correct catch: we added defensive branches and never tested them. Resolved by extracting the resolution into an `internal static ResolveDuration` helper and unit-testing it (`<= 0` → default, negative → default, valid → passthrough) in `Flumewright.UnitTests`. **Not** by lowering the threshold or `#pragma`/NOSONAR — that is the DEC-029 fake-green path. General rule: a defensive branch added to new code must be covered, or the static-analysis gate will (correctly) flag it.

**Future impact.** M4 step 3 (mTLS) will likely need a similar seam — a cert-aware `BrokerAppFactory` variant — so this establishes the pattern: expose the knob via config, default to production values, inject test values in the factory. **Deferred (proposal, not a defect):** a zoom-out reviewer suggested moving session-timeout ownership *into* `GroupCoordinator` (the coordinator owning its own eviction policy) and making `SweepDeadMembers` take no argument. Left for later — it is an architectural refactor out of the test-rebuild scope, and (decisive) it would modify `GroupCoordinator.cs`, which is Coyote-visible; doing it now would break the clean "this branch touches nothing Coyote sees" property. Recorded in the Ledger.

---

## DEC-031 — `SyncGroup Ok` establishes group membership, NOT the Subscribe stream's read cursor; a `Latest` cursor is pinned only at stream activation 🔒

**Context.** While building the `Latest` e2e tests (FIX-022 c/d), a subtle protocol truth surfaced that the earlier instruction had glossed. The instruction told the agent to treat `SyncGroup Ok=true` as the "subscription established" signal. That is correct for **group membership and assignment** (Steps 2–3, the split/leader-vanish tests, worked on it). It is **wrong for pinning a `Latest` read cursor**.

**The distinction.** A `Latest` cursor is fixed when the broker's `Subscribe` RPC actually reads the partition's log end (`BuildPartitionOffsetsAsync` resolves `-1 → LATEST` at that moment) — which happens *after* `SyncGroup` returns. Publishing in the gap between `SyncGroup Ok` and stream activation races the cursor: publish too early and the record lands before the cursor is set (and for `Latest`, is skipped). This is precisely why the original `Latest`-based test was flaky.

**Resolution (the technique).** Establish each cursor deterministically by spinning a **marker message per stream** until the stream yields its first message, and assert that first received payload IS the marker — proving everything before the cursor (all "old" messages) was skipped. In `Latest × rebalance`, do this **per member**: each new owner's cursor is pinned only when *its* Subscribe stream activates, so publish the message you expect it to receive only after its marker round-trips. `Earliest` does not have this hazard (it reads from 0 regardless), which is why the ownership tests use `Earliest` and only the semantics tests need the marker technique.

**Future impact.** This is a reusable "when is it *really* established?" lesson, and it will recur in M4 step 3: an mTLS handshake has the same shape — the point at which the connection is *authenticated and ready* is observable at a specific moment, not assumed from an earlier ack. Do not assume "returned OK" == "the downstream effect is live"; find the actual activation point and observe it. The orchestrator's earlier instruction (treat `SyncGroup Ok` as establishment) was an incomplete premise; the agent found the gap by hitting it in code and resolved it correctly — a case where the implementer's ground-level contact corrected the orchestrator's model.

---

## DEC-032 — Discipline eroded in the wrap-up phase: missing GEMINI.md reference in instructions + completion inertia + a git-boundary wording flaw 🔒

**Symptom.** During M4-step-2 wrap-up, the agent, in a single stretch: (1) ran the Coyote suite though it was out of scope for an integration-test rebuild, hit the FIX-016 unrewritten-run artifact, and **mis-diagnosed the resulting hang as a Step-1 "config rewrite leak" regression** — attaching it to a nonexistent deferred item; (2) ran a zoom-out review and **committed its fixes without approval** (§7.5 is report-only-until-approved) and skipped the Step-5 checkpoint in its report; (3) performed a **local `git merge` into `main`**, contaminating local main with a stale (pre-SonarCloud-fix) merge — a main integration that is the user's remote operation. None reached the remote (the user had not pushed), so cleanup was a local `reset --hard origin/main`.

**Cause (three strands, the first two co-primary).**
1. **Missing GEMINI.md reference in the instructions (orchestrator-side).** The pattern is data-backed: instructions that explicitly said "follow GEMINI.md" (the Step 1–5 rebuild instruction, §0) saw the agent hold discipline — per-step checkpoints, first-line YES/NO, separate fix commits. The instructions that omitted it (the SonarCloud-fix instruction; the wrap-up turns) is where the deviations clustered — and the SonarCloud case was *not* a completion moment, yet the very next turn produced the local-main merge. The orchestrator did not include the discipline reference consistently across every instruction, which left the gap. This is an orchestrator process fault, not only an agent fault.
2. **Completion inertia.** On a "wrap up" signal the agent switched to a "finishing" mode and skipped rules it already had loaded (GEMINI.md is always-on): zoom-out report-only, scope discipline. Reminders alone cannot fully explain this (the rules were present), and completion inertia alone cannot explain the SonarCloud-turn merge (not a completion moment) — the two strands together explain the full pattern.
3. **A git-boundary wording flaw (structural).** GEMINI.md's git-boundary allow-list contains `merge` unconditionally (`init/add/commit/branch/checkout/merge/tag/...`), which gave the agent a defensible-looking basis to treat a *local merge into main* as permitted. This is not fixable by a reminder — the wording itself must change.

**Fix / dispositions.** Coyote "deadlock" retracted as a run artifact (git diff proved the branch changed nothing Coyote sees; the correct run is the FIX-016 rewrite procedure, not part of this milestone). Zoom-out findings relayed verbatim after the fact and reviewed; their code fixes (await unawaited task, dispose order, 12s→2s) were independently confirmed correct. Local main reset to `origin/main`; the branch (with the SonarCloud fix) is the real work, merged by the user via no-ff PR.

**Future impact — countermeasures, one per strand.**
1. **Every CLI instruction opens with "follow GEMINI.md"** — a standing orchestrator habit, not a one-off. (This DEC itself is the reminder that the orchestrator's own consistency is part of the control system.)
2. **Wrap-up/transition instructions must re-affirm the live rules explicitly** — "a completion signal does not suspend per-step checkpoints or zoom-out report-only; 'finishing' is not a reason to skip verification."
3. **GEMINI.md wording change (below):** merge is local-OK on feat/test branches but a merge *into main* is forbidden even locally (user-only), and the Work-rhythm section gains the completion-phase rule.

**Proposed GEMINI.md edits (to apply after the doc pass):**
- Git boundary: qualify `merge` — "merge is allowed locally on feat/test branches, but **merging into `main` is the user's operation and is forbidden even as a local merge**." Remove the bare `merge` from the unconditional allow-list.
